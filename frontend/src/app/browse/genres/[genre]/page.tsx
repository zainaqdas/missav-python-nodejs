...
export default function GenreVideosPage({ params }: GenrePageProps) {
  ...
  const fetchPage = async (pageNum: number) => {
    ...
    try {
      const browseRes = await fetch(`/api/browse?type=genre&slug=${encodeURIComponent(genre)}&page=${pageNum}`);
      if (!browseRes.ok) throw new Error('Failed to fetch genre page');
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-6 py-8">
        {/* Breadcrumb */}
        <div className="mb-8">
          <Link href="/" className="text-sm text-gray-500 hover:text-primary-300 transition-colors">
            Home
          </Link>
          <span className="text-sm text-gray-600 mx-2">/</span>
          <Link href="/browse/genres" className="text-sm text-gray-500 hover:text-primary-300 transition-colors">
            Genres
          </Link>
          <span className="text-sm text-gray-600 mx-2">/</span>
          <span className="text-sm text-gray-300 capitalize">{decodedGenre}</span>
        </div>

        <h1 className="text-3xl font-bold text-white capitalize mb-8">{decodedGenre}</h1>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-gray-500">No videos found for this genre.</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
